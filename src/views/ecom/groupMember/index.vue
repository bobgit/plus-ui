    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['ecom:groupMember:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['ecom:groupMember:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['ecom:groupMember:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['ecom:groupMember:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" border :data="groupMemberList" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column label="团员ID" align="center" prop="memberId" v-if="true" />
        ...
        <el-table-column label="操作" align="center" fixed="right"  class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ecom:groupMember:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['ecom:groupMember:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
import { listGroupMember, getGroupMember, delGroupMember, addGroupMember, updateGroupMember } from '@/api/ecom/groupMember';
import { GroupMemberVO, GroupMemberQuery, GroupMemberForm } from '@/api/ecom/groupMember/types';
...
const data = reactive<PageData<GroupMemberForm, GroupMemberQuery>>({
  ...
  rules: {
    memberId: [
      { required: true, message: "团员ID不能为空", trigger: "blur" }
    ],
    recordId: [
...
const handleUpdate = async (row?: GroupMemberVO) => {
  ...
  const res = await getGroupMember(_memberId);
...
const handleDelete = async (row?: GroupMemberVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除拼团团员编号为"' + _memberIds + '"的数据项？').finally(() => loading.value = false);
  await delGroupMember(_memberIds);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('ecom/groupMember/export', {
    ...queryParams.value
